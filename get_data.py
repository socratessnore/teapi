...
class Authenticator:
    ...
    # Excel
    EXCEL_GENERATE_URL = BASE_URL + 'Reporting/CustomerConsumption/GenerateExcelFile?start={}&end={}&selectedTimeSpan=day'
    EXCEL_DOWNLOAD_URL = BASE_URL + 'Reporting/CustomerConsumption/DownloadExcelFile?identifier={}'
    ...
    def _refresh_form_token(self):
        ...
        if request.status_code == 200:
            auth_token = re.search(
                b'name=\"__RequestVerificationToken\" .*? value=\"(.*?)\"',
                request.content
            )
            if auth_token:
    ...
    def _fetch_excel_identifier(self):
        ...
        response = self.REQUEST.post(
            self.EXCEL_GENERATE_URL.format(
                first_day,
                first_day
            ),
            headers={
